...
export default async function Page(
  ...
  const page = girlsServerSource.getPage(params.slug);
  if (!page) notFound();

  const MDX = page.data.body;
  const markdownUrl = getGirlsServerPageMarkdownUrl(page).url;
      ...
      <DocsBody>
        <MDX
          components={getMDXComponents({
            ...
            a: createRelativeLink(girlsServerSource, page),
          })}
        />
      </DocsBody>
    </DocsPage>
...
export async function generateStaticParams() {
  return girlsServerSource.generateParams();
}
...
export async function generateMetadata(
  ...
  return {
    ...
    openGraph: {
      images: getGirlsServerPageImageUrl(page).url,
    },
  };
}
